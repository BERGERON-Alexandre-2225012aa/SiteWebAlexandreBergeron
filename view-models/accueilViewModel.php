<?php

function body() { ?>
    <div class="accueilDiv reveal">
        <img class="accueilImg left" src="../ressources/images/alexandre_lavandes.jpg">
        <label class="accueilLabel right">
            Bonjour, je suis Alexandre Bergeron !
            <br>
            <br>J'ai 18 ans, et je suis passionné d'informatique.
            <br>
            <br><label class="gradientWord">Motivé</label>,
            <label class="gradientWord">Rigoureux</label> et
            <label class="gradientWord">Organisé</label> dans mon travail,
            je suis un atout primordial pour les équipes avec qui je travaille.
        </label>
    </div>
    <div class="accueilDiv reveal">
        <label class="accueilLabel left">
            Je suis actuellement en 2ème année de BUT informatique à l'Institut Universitaire de Technologies d'Aix-en-Provence.
        </label>
        <img class="accueilImg right" src="../ressources/images/iut.jpg">
    </div>
    <div class="accueilDiv reveal">
        <img class="accueilImg left" src="../ressources/images/codage.jpg">
        <label class="accueilLabel right">
            Langages maîtrisés :
            <br>
            <br>
            <br>
            <div class="accueilSkillDiv">
                <label>Java</label>
                <div class="skillStarsDiv">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                </div>
            </div>
            <div class="accueilSkillDiv">
                <label>C++</label>
                <div class="skillStarsDiv">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                </div>
            </div>
            <div class="accueilSkillDiv">
                <label>Python</label>
                <div class="skillStarsDiv">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                </div>
            </div>
            <div class="accueilSkillDiv">
                <label>HTML</label>
                <div class="skillStarsDiv">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                </div>
            </div>
            <div class="accueilSkillDiv">
                <label>CSS</label>
                <div class="skillStarsDiv">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                </div>
            </div>
            <div class="accueilSkillDiv">
                <label>JavaScript</label>
                <div class="skillStarsDiv">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                </div>
            </div>
            <div class="accueilSkillDiv">
                <label>PHP</label>
                <div class="skillStarsDiv">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                </div>
            </div>
            <div class="accueilSkillDiv">
                <label>SQL</label>
                <div class="skillStarsDiv">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                    <img src="../ressources/images/etoile-jaune.png" width="60">
                </div>
            </div>

        </label>
    </div>

<?php } ?>